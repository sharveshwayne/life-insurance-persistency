...
FEATURES = [
    ...
    "agent_tenure_days",
    "agent_persistency",
    "last_6_month_submissions",
    "average_premium",
    "is_reinstated",
    "prev_persistency",
    "num_complaints",
    "target_completion_perc",
    "has_contacted_in_last_6_months",
    "credit_score",
    "time_to_issue",
    "prem_to_income_ratio",
]
...
def read_data(INPUT_FILEPATH, INPUT_FILENAME) -> pd.DataFrame:
    input_df = pd.read_parquet(
        os.path.join(INPUT_FILEPATH, INPUT_FILENAME), engine="pyarrow"
    )
        ...
        [
            (
                ...
                mdi.MeanMedianImputer(
                    imputation_method="median", variables=MISSING_COLS
                ),
            ),
            (
                "onehot_encoder",
                ce.OneHotEncoder(
                    top_categories=None, variables=ONE_HOT_COLS, drop_last=True
                ),
            ),
...
def main():
    ...
    X_train_trf, X_test_trf, y_train, y_test, model_input_pipe = crate_train_test(
        clean_df
    )
